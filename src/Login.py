...
class Login(tk.Tk):
    ...
    def create_login_window(self):
        ...
        self.login_button = ttk.Button(self, text="Login", width=8, command=lambda:self.loginButtonPressed(self.username_entry.get(), self.password_entry.get()))
        self.login_button.grid(column=0, row=3, columnspan=2, padx=5, pady=5)

        self.register_button = ttk.Button(self, text="Register", width=8, command=lambda:self.registerButtonPressed(self.username_entry.get(), self.password_entry.get()))
        self.register_button.grid(column=0, row=4, columnspan=2, padx=5, pady=5)
