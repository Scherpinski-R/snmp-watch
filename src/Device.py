...
class Device:
    def __init__(self, deviceAlias, deviceIp, authCred, cryptCred):
        self.alias  = deviceAlias
        self.ipAddr = deviceIp
        
        self.auth   = authCred              # (user, pass) for authenticating user
        self.priv   = privCred              # (user, pass) for cryptographing data
        
        self._session = None 

        #Const:
        #TO-DO: add ifSpeed if there is time left
        parameterNameList = ['hrStorageSize.1', 'hrStorageSize.3', 'hrSystemProcesses'] # RAM - SWAP - Number Processes
        
        self.deviceList = []
        self.populateParameterList()

    def createSession(self):
        self._session = Session( hostname=self.ipAddr, version=3, security_level="auth_with_privacy",
                        auth_protocol="MD5", security_username=self.auth[0], auth_password=self.auth[1]),
                        privacy_protocol="DES", privacy_password=self.priv[1])
